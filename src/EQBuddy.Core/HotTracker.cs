...
/// <summary>One of your heal-over-time spells currently ticking on a named target.</summary>
...
///
/// A chip appears when your HoT starts healing someone: "You healed Daggo over time for 61
/// hit points by Blossoming Heal." is the only line that ever announces a HoT is running.
/// Unlike a mez, a HoT announces neither its landing nor its fade — there is not one "Your
/// Blossoming Heal spell has worn off" line in 690k lines of eqlog_Daggo_freeport, while
/// Mesmerization fades 1197 times in the same file — so the first tick both opens the chip
/// (naming the target) and every later tick of the same cast sustains it.
///
...
///      measures short and nothing measures long.
...
///      it is not.
///
/// One limitation follows directly from the log, not from a choice made here: a HoT cast on
/// a target already at full health heals nothing, so it logs nothing, so it never ticks and
/// never chips. Not one of the roughly 3700 HoT tick lines in eqlog_Daggo_freeport carries a
/// 0 hit-point amount, and there is no fallback either — "You have targeted X" appears 0
/// times in that same 690k-line log, so a cast line cannot even be paired with a target by
/// other means. That HoT is genuinely invisible to the log, and therefore to this tracker;
/// there is no chip for it, and manufacturing one would mean guessing a target, which is
...
public sealed class HotTracker
{
    ...
    private bool OnTick(HealEvent tick)
    {
        ...
        // This tick opens a NEW series — a fresh cast, or a recast after the last one
        // finished (see Continues).
        //
    ...
    private bool OnTrigger(HealEvent burst)
    {
        ...
        if (!_active.Remove(Key(burst.Target, spell), out var entry))
            // No series on this target: the target was already at full health when this
            // burst finally landed (see the class header), the HoT started before this
            // tracker was watching, or this Trigger belongs to a spell we never saw tick.
            // Either way there is no chip to close and nothing to learn.
            return false;
